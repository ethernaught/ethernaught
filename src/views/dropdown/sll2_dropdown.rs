...
impl Sll2Dropdown for Dropdown {
    ...
    fn from_sll2_frame(hex_editor: &HexEditor, actions: &SimpleActionGroup, layer: &Sll2Frame, offset: usize) -> Self {
        ...
        _self.list_box.add(&create_row(format!("{}:", layer.get_title("if_index")), layer.get_value("if_index")));
        _self.list_box.add(&create_row(format!("{}:", layer.get_title("data_link_type")), layer.get_value("data_link_type")));
        _self.list_box.add(&create_row(format!("{}:", layer.get_title("packet_type")), layer.get_value("packet_type")));
        _self.list_box.add(&create_row(format!("{}:", layer.get_title("address_length")), layer.get_value("address_length")));
        _self.list_box.add(&create_row(format!("{}:", layer.get_title("address")), layer.get_value("address")));
